...
def _scale_channels(base_channels, width, max_channels):
    channels = []
    for ch in base_channels:
        scaled = int(ch * width)
        scaled = max(8, int(math.ceil(scaled / 8.0) * 8))
        channels.append(min(scaled, max_channels))
    return channels

def _scale_repeats(base_repeats, depth):
    return [max(int(round(r * depth)), 1) for r in base_repeats]

def _build_legacy_yolo(scale, num_classes):
    scales = {
        'n': (0.50, 0.25, 1024),
        's': (0.50, 0.50, 1024),
        'm': (0.50, 1.00, 512),
    ...
    channels = [3] + _scale_channels([64, 128, 256, 512, 1024], width, max_channels)
    repeats = _scale_repeats([2, 2, 2, 2, 2, 2], depth)
    csp = [False, True]
    return YOLO(channels, repeats, csp, num_classes)

def yolo_v11_n(num_classes: int = 80):
    return _build_legacy_yolo('n', num_classes)

def yolo_v11_s(num_classes: int = 80):
    return _build_legacy_yolo('s', num_classes)

def yolo_v11_m(num_classes: int = 80):
    return _build_legacy_yolo('m', num_classes)

def yolo_v11_l(num_classes: int = 80):
    return _build_legacy_yolo('l', num_classes)

def yolo_v11_x(num_classes: int = 80):
    return _build_legacy_yolo('x', num_classes)

def build_model(model_name: str, num_classes: int = 80):
    builders = {
